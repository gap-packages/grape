...
This manual describes the {\GRAPE} (Version~4.4) package for computing
...
L.H. Soicher, The {GRAPE} package for {GAP}, Version~4.4, 2011,
...
From {\GAP}~4.5, {\GRAPE} is included in the standard {\GAP} distribution.
If appropriate binary files are not included, you will need to perform
compilation of B.D.~McKay's dreadnaut/nauty programs included with
{\GRAPE}. For example, in a Unix environment, after installing {\GAP}, go
to the {\GRAPE} home directory (usually the directory `pkg/grape' of the
{\GAP} home directory), and run `./configure <path>', where <path> is the
path of the {\GAP} home directory.  So for example, if the {\GRAPE} home
directory is the `pkg/grape' directory of the {\GAP} home directory, run
