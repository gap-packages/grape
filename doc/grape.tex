...
This manual describes the {\GRAPE} (Version~4.5) package for computing
...
L.H. Soicher, The {GRAPE} package for {GAP}, Version~4.5, 2011,
...
The {\GRAPE} package files are included in the standard {\GAP}~4.5
distribution, so you do not need to download and unpack an archive for
{\GRAPE} unless you need to install the package locally or are installing
an upgrade of {\GRAPE} to an existing installation of {\GAP} (see the main
{\GAP} reference section "Reference: Installing a GAP Package").  If you
do need to download {\GRAPE}, you can find archive files for the package
in various formats at \URL{http://www.gap-system.org/Packages/grape.html},
and then your archive file of choice should be downloaded and unpacked
in the `pkg' subdirectory of an appropriate {\GAP} root directory (see
the main {\GAP} reference section "Reference: GAP Root Directory").

Regardless of whether you needed to download and unpack an archive file,
you will need to perform compilation of B.D.~McKay's nauty/dreadnaut
programs included with {\GRAPE}, and this can only be done on a Unix
system.  After installing {\GAP}, go to the {\GRAPE} home directory
(usually the directory `pkg/grape' of the {\GAP} home directory),
and run `./configure <path>', where <path> is the path of the {\GAP}
home directory.  So for example, if the {\GRAPE} home directory is the
`pkg/grape' directory of the {\GAP} home directory, run
...
this configuration process will only work for the *last* architecture for
which {\GAP} was compiled. Therefore, you should always follow the above
procedure to install the {\GRAPE} binaries immediately after compiling
{\GAP} on a given architecture.
